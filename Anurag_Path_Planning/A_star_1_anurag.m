...
% array to store the coordinates of the map and the objects
MAP=2*(ones(MAX_X,MAX_Y));

% interactive obstacle selection
useImageMap = true;
mapfile = 'environment_map.png';

if useImageMap && isfile(mapfile)
    img = imread(mapfile); % obstacle image
    if size(img,3) == 3
        img = rgb2gray(img);
    end
    img = imbinarize(img); 
    img = imresize(img, [MAX_X, MAX_Y], 'nearest'); % grid size fitting
    ...
    for i = 1:MAX_X
        for j = 1:MAX_Y
            if img(i,j) == 0
                ...
                MAP(i,j) = 2; % blank space
            end
        end
    end

    figure; imshow(img);
    title("Loaded environment from image")
else
    disp("No valid map/image found -->  Switching to manual obstacle placement.")
    useImageMap = False;
end
...
% target coordinates
xTarget=xval;%X Coordinate 
yTarget=yval;%Y Coordinate 

%location of the target (map)
MAP(xval,yval)=0;
...
% assuming a random size for the robot, so inflating obstacles to the robot
robot_radius = 0.3; %meters
wheel_radius = 0.1;
safety_margin = robot_radius + wheel_radius;

inflate_pixels = ceil(safety_margin*10);

% converting MAP to binary image
map_img = MAP == -1;
se = strel('disk', inflate_pixels);
map_inflated = imdilate(map_img, se);

% updated map
MAP(map_inflated) = -1;
figure; imshow(map_inflated);
title("Obstacle map with safety margin for Robot")
