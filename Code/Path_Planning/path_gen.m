...
% matching simscape scaling for map
cellSize_plot = 0.5;
imshow(display_map, 'InitialMagnification', 'fit', 'XData', [0, MAX_Y*cellSize_plot],'YData',[0, MAX_X*cellSize_plot]);
hold on;
axis([0 MAX_Y*cellSize_plot 0 MAX_X*cellSize_plot]);
axis on; grid on;
xlabel('x [m]'); ylabel('y [m]');

% from cell units to metres (realistic approach for pure pursuit)
plot((yStart-0.5)*cellSize_plot, (xStart-0.5)*cellSize_plot,'bo', 'MarkerSize', 8, 'LineWidth', 2);
text((yStart+1)*cellSize_plot, (xStart-0.5)*cellSize_plot,'Start', 'Color', 'blue');
plot((yTarget-0.5)*cellSize_plot, (xTarget-0.5)*cellSize_plot,'gd', 'MarkerSize', 8, 'LineWidth', 2);
text((yTarget+1)*cellSize_plot, (xTarget-0.5)*cellSize_plot, 'Target', 'Color', 'green');
...
if xNode == xTarget && yNode == yTarget
    ...
    % animated final path plot (rectified)
    j = size(Optimal_path, 1);
    p = plot((Optimal_path(1,2)-0.5)*cellSize_plot, (Optimal_path(1,1)-0.5)*cellSize_plot, 'bo', 'MarkerSize', 8, 'LineWidth', 2);

    for idx = 2:j
        pause(0.05);
        set(p, 'XData', (Optimal_path(idx,2)-0.5)*cellSize_plot, 'YData', (Optimal_path(idx,1)-0.5)*cellSize_plot);
        drawnow;
    end

    plot((Optimal_path(:,2)-0.5)*cellSize_plot, (Optimal_path(:,1)-0.5)*cellSize_plot, 'c-', 'LineWidth', 2.5);
    title(sprintf('Path Found  |  scale = %.1f  |  buffer = %d cells  |  %d waypoints', scale_factor, robot_radius, size(waypoints,1)));
